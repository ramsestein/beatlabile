...
def fig1_strobe(results_dir: Path) -> None:
    ...
    boxes = [
        (0.5, 0.95, "Clínic ICU\n~650 patients total"),
        (0.5, 0.82, "QC: ~150 complete records (≥6 h)\n+ ~500 valid fragments (≥2 h)"),
        (0.5, 0.68, "Act 1 — Development\nn=150 complete + fragments\n3 models × 3 approaches"),
        (0.5, 0.50, "Act 2 — Blind validation MIMIC-IV\nn=150 | 3 models applied without refit"),
        (0.5, 0.32, "Act 3a — Signal-only revalidation VitalDB\nn=150"),
        (0.5, 0.16, "Act 3b — Sufficiency test M1–M6\n+ Subgroups (age, sex, ASA, BMI)"),
    ]
    ...
    ax.set_title("Fig 1 — STROBE flow: BeatLabile Study (three acts)", fontsize=13, fontweight="bold")
...
def fig2_case_star(metrics_ts: pd.DataFrame | None, results_dir: Path) -> None:
    ...
    for ax, (col, label, color, threshold) in zip(
        ...
        if threshold is not None:
            ax.axhline(threshold, color="gray", lw=1, ls="--", label=f"Threshold {threshold}")
        ax.axvline(0, color="red", lw=2, ls="--", label="ELH onset")
        ax.axvspan(-20, -5, alpha=0.1, color="orange", label="Prediction window")
        ax.set_ylabel(label, fontsize=9)
        ax.legend(fontsize=7, loc="upper left")

    axes[-1].set_xlabel("Time to event (min)", fontsize=10)
    fig.suptitle("Fig 2 — Example: physiological metrics before hypotension episode", fontsize=12)
...
def fig3_roc_by_horizon(
    ...
    for j, horiz in enumerate(horizons):
        ...
        for i, etype in enumerate(event_types):
            ...
            ax.set_xlabel("1 - Specificity" if i == len(event_types) - 1 else "")
            ax.set_ylabel("Sensitivity" if j == 0 else "")
            ax.set_title(f"{EVENT_LABELS[etype]}\n{horiz}", fontsize=9)
            ax.legend(fontsize=8)

    fig.suptitle("Fig 3 — ROC curves by prediction horizon and event type", fontsize=12)
    plt.tight_layout()
...
def fig4_forest_auc(
    ...
    fig_dir = _setup_figure_dir(results_dir)
    event_types = ["hypotension", "hypertension", "variability"]
    cohorts = ["Clínic (Act 1)", "MIMIC-IV (Act 2)", "VitalDB (Act 3)"]
    ...
    ax.set_title("Fig 4 — Signal-only AUC across three cohorts (GLMM model)", fontsize=11)
    plt.tight_layout()
...
def fig5_milp_tree(milp_tree, results_dir: Path, event_type: str = "hypotension") -> None:
    ...
    ax.set_title(
        f"Fig 5 — Optimal MILP decision tree: {EVENT_LABELS.get(event_type, event_type)}",
        fontsize=11,
    )
...
def fig6_bootstrap_stability(stability_result, results_dir: Path, event_type: str = "hypotension") -> None:
    ...
    ax1.axvline(0.80, color="red", lw=1.5, ls="--", label="Stability (80%)")
    ax1.set_xlabel("Bootstrap selection frequency")
    ...
    if dist_data:
        ...
        ax2.set_ylabel("Optimal threshold (bootstrap)")
        ax2.set_title("Threshold distribution (bootstrap)")
    else:
        ax2.text(0.5, 0.5, "Insufficient data", ha="center", va="center",
                 transform=ax2.transAxes)

    fig.suptitle(
        f"Fig 6 — MILP tree bootstrap stability: {EVENT_LABELS.get(event_type, event_type)}",
        fontsize=11,
    )
    plt.tight_layout()
...
def fig7_sufficiency(act3_results: dict, results_dir: Path) -> None:
    ...
    fig.suptitle("Fig 7 — Sufficiency test: ΔAUC M1–M6 by event type", fontsize=11)
    plt.tight_layout()
...
def fig8_benchmark(act1_results: dict, results_dir: Path) -> None:
    ...
    ax.set_title("Fig 8 — Benchmark: interpretable models vs ML", fontsize=11)
    plt.tight_layout()
...
def fig9_subgroups(act3_results: dict, results_dir: Path) -> None:
    ...
    fig.suptitle("Fig 9 — VitalDB subgroup analysis (AUC M1)", fontsize=11)
    plt.tight_layout()
...
def fig10_decision_curves(
    ...
    for ax, etype in zip(axes, event_types):
        ...
        ax.plot(thresh_range, nb_all, "k--", lw=1, label="Treat all")
    ...
    fig.suptitle("Fig 10 — Decision Curve Analysis by event type", fontsize=11)
    plt.tight_layout()
