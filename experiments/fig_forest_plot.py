...
PANEL_TITLES = {
    ...
    "hypertension": "Intraoperative hypertension",
}
...
def _draw_forest_panel(ax: plt.Axes, fe_mean: np.ndarray, fe_sd: np.ndarray,
    ...
    ax.set_xlabel("Posterior coefficient (standardized scale)", fontsize=9)
    ax.set_title(title, fontsize=11, fontweight="bold", pad=8)
    ax.spines[["top", "right"]].set_visible(False)

    ci_note = "(mean ± 1.96·posterior SD)" if has_se else "(posterior mean — SD unavailable)"
...
def run_fig6() -> None:
    ...
    fig.suptitle(
        "Fig. 6 — GLMM coefficients: beat-by-beat BP autonomic metrics\n"
        "Parsimonious model (8 features) — Clínic ICU cohort",
    ...
    # Legend
    red_patch  = mpatches.Patch(color="#c0392b", alpha=0.78, label="Coef > 0: ↑ risk")
    blue_patch = mpatches.Patch(color="#2980b9", alpha=0.78, label="Coef < 0: ↓ risk")
