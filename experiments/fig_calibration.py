...
TITLES  = {"hypotension": "Hypotension", "hypertension": "Hypertension"}

# Cohort specs: (label, dir, linestyle, color, marker)
COHORTS = [
    ("Clínic (development)",  ACT1_DIR, "-",  "#2c7bb6", "o"),
    ("MIMIC (direct transfer)", ACT2_DIR, "--", "#d7191c", "s"),
]
...
def _draw_calibration_panel(ax: plt.Axes, etype: str,
    ...
    ax.set_xlabel("Predicted probability (decile mean)", fontsize=9)
    ax.set_ylabel("Observed rate (decile mean)", fontsize=9)
...
def run_fig7() -> None:
    ...
    fig.suptitle(
        "Fig. 7 — GLMM model calibration: predicted vs observed probability\n"
