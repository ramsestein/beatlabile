...
def _collect_nodes(node, x=0.5, y=0.92, dx=0.22, depth=0, nodes=None, edges=None):
    ...
    if not is_leaf:
        ...
        edges.append(dict(parent=node_id, child=left_id, label="SÍ  (≤)"))
...
def _draw_tree(ax, milp, title: str, outcome_label: str,
    ...
    # ── Legend ──
    ax.legend(handles=[
        mpatches.Patch(fc=C_HIGH, label="Riesgo alto"),
        mpatches.Patch(fc=C_LOW,  label="Riesgo bajo"),
...
def run_fig5() -> None:
    ...
    plt.suptitle(
        "Fig. 5 — Reglas de decisión MILP: métricas autonómicas de PA beat-by-beat",
